 ...
 */
 ...
 func (db *MongoDB) initData() error {
	 ...
	 if count < 1 {
		 ...
		 user = models.User{bson.NewObjectId(), "last", "first", "admin", 
		 time.Time(time.Now().Local().UTC()), time.Time(time.Now().Local().UTC()), "xxx", "xxx", 
		"admin@gmail.com", "california", "i'm admin", "+628888", "USA", true, false, "img.com/img.png",
		"img.com/img.png"}
